...
def main():
    ...
    analyze_parser.add_argument(
        "--model", "-m", help="Path to .kg model file (v0.1.1+)"
    )
    ...
    save_parser = subparsers.add_parser(
        "save", help="Save model as single package file"
    )
    save_parser.add_argument(
        "--output", "-o", required=True, help="Output path for model package"
    )
    save_parser.add_argument(
        "--rust", action="store_true", help="Use Rust acceleration"
    )
...
def handle_save(args):
    """Handle save command - package model into single file"""
    print("Initializing MorphAnalyzer for model packaging...")
    try:
        analyzer = MorphAnalyzer(
            use_double_array=True,
            use_sejong=True,
        ...
        # Save model as package
        output_path = analyzer.save_model(args.output)
        print(f"\n✓ Model successfully packaged to: {output_path}")
